...
- [x] Custom SMTP via Resend — `supabase/config.toml` + `SMTP_SETUP.md`
- [x] Auth email templates (ภาษาไทย, brand สอดคล้อง) — `apps/web/src/lib/email-templates.ts`
  - [x] Confirm signup email — `getConfirmationEmailTemplate()`
  - [x] Magic link email — `getMagicLinkEmailTemplate()`
  - [x] Reset password email — `getResetPasswordEmailTemplate()`
  - [x] Change email confirmation — `getEmailChangeTemplate()`
...
## Phase 6: SaaS Readiness — Billing, Limits & Admin
> เป้าหมาย: ระบบพร้อมขายเป็น SaaS ได้ มี billing, usage limits, admin dashboard

### Database Schema
- [ ] Migration: `plans` table (name, slug, price_monthly, price_yearly, limits JSON, features JSON, stripe_price_id)
- [ ] Migration: `subscriptions` table (workspace_id, plan_id, status, stripe_subscription_id, current_period_start, current_period_end, cancel_at)
- [ ] Migration: `usage_records` table (workspace_id, metric, count, period_start, period_end)
- [ ] Migration: `invoices` table (workspace_id, stripe_invoice_id, amount, status, paid_at, invoice_url)
- [ ] Migration: `api_keys` table (workspace_id, key_hash, name, scopes, last_used_at, expires_at)
- [ ] Migration: `notifications` table (user_id, workspace_id, type, title, body, read_at, action_url)
- [ ] Migration: `webhook_endpoints` table (workspace_id, url, events, secret, is_active, last_triggered_at)
- [ ] Migration: `webhook_logs` table (webhook_endpoint_id, event, payload, response_status, response_body, triggered_at)
- [ ] RLS policies ทุก table ใหม่

### Billing & Subscription (Stripe)
- [ ] Stripe SDK integration (`@stripe/stripe-js` + `stripe` server-side)
- [ ] Plan tiers: Free / Starter ($49) / Pro ($99) / Agency ($199)
- [ ] Checkout session (สมัคร plan) — Stripe Checkout
- [ ] Customer portal (เปลี่ยน plan, ยกเลิก, ดู invoices) — Stripe Customer Portal
- [ ] Webhook handler (`/api/webhooks/stripe`) — invoice.paid, subscription.updated, subscription.deleted, payment_intent.failed
- [ ] Trial period (14 วัน free Pro) — Stripe trial
- [ ] Proration เมื่อ upgrade/downgrade กลางรอบ
- [ ] Invoice history page — ดู/ดาวน์โหลด invoices
- [ ] Payment method management — เพิ่ม/เปลี่ยนบัตร
- [ ] Dunning management — retry failed payment + email แจ้ง
- [ ] Grace period (3 วัน) หลัง payment fail ก่อน downgrade

### Usage Limits & Quotas
- [ ] Usage metering service — นับ leads created, emails sent, campaigns, sequences per workspace per month
- [ ] Limits enforcement middleware (tRPC) — เช็ค quota ก่อนทุก action ที่มี limit
- [ ] Plan limits config:
  - [ ] Free: 50 leads/mo, 500 emails/mo, 1 campaign, 0 sequences
  - [ ] Starter: 500 leads/mo, 5,000 emails/mo, 5 campaigns, 3 sequences
  - [ ] Pro: 2,000 leads/mo, 20,000 emails/mo, unlimited campaigns, unlimited sequences
  - [ ] Agency: 10,000 leads/mo, 100,000 emails/mo, unlimited all
- [ ] Usage dashboard (แสดง current usage vs limit, progress bars)
- [ ] Usage alert emails (80%, 100% ของ quota)
- [ ] Overage handling — block action + แสดง upgrade prompt
- [ ] Monthly reset cron job (reset usage counters)

### Onboarding Flow
- [ ] Welcome wizard (step-by-step) — ชื่อ workspace → เลือก plan → ตั้งค่า domain → invite team
- [ ] Setup checklist (dashboard widget) — ✓ verify domain ✓ create template ✓ search first leads
- [ ] Interactive product tour (tooltips / modal guide)
- [ ] Sample data option (demo leads, demo template)

### Notification System
- [ ] In-app notifications (bell icon + dropdown)
- [ ] Notification types: billing, usage limit warning, campaign complete, bounce alert, team invite, system updates
- [ ] Mark as read / mark all read
- [ ] Email notification preferences (settings page) — เลือกว่าจะรับ email แจ้งเตือนอะไรบ้าง
- [ ] Real-time push via Supabase Realtime

### Admin Dashboard (Internal — สำหรับเราดูแลระบบ)
- [ ] Route: `/admin` (แยก layout, super-admin only)
- [ ] Metrics overview: total users, workspaces, MRR, churn rate
- [ ] User management: list, search, suspend, impersonate
- [ ] Workspace management: list, usage, plan, manual override
- [ ] Subscription management: view all subscriptions, manual plan change
- [ ] System health: API response times, error rates, queue status
- [ ] Feature flags management (enable/disable features per plan/workspace)
- [ ] Announcement system (broadcast message ถึง users ทุกคน)

### Security & Compliance
- [ ] Two-Factor Authentication (2FA) — TOTP (Google Authenticator)
- [ ] Session management — list active sessions, revoke
- [ ] Login history & audit trail
- [ ] PDPA compliance — data export (download all my data), data deletion request
- [ ] GDPR compliance — consent management, right to erasure
- [ ] Rate limiting per user/IP (API routes)
- [ ] CSRF protection review
- [ ] Security headers (CSP, HSTS, X-Frame-Options)
- [ ] Dependency vulnerability scanning (npm audit, pip audit)

### Error Monitoring & Observability
- [ ] Sentry integration (Next.js + Python FastAPI)
- [ ] Error tracking with source maps
- [ ] Performance monitoring (transaction tracing)
- [ ] Structured logging (JSON logs) — Python FastAPI
- [ ] Health check endpoints (`/api/health`, `/api/v1/health`)
- [ ] Uptime monitoring (external service — e.g., BetterUptime)
- [ ] Alert rules — error spike, high latency, failed jobs

---

## Phase 7: Scale, Integrations & Advanced
> เป้าหมาย: Scale ระบบ, Public API, Email migration, White-label

### Email Service Migration (Resend → AWS SES)
...
- [ ] Handle SES bounce/complaint ผ่าน SNS → Webhook → Supabase
- [ ] Switch campaign → AWS SES เมื่อ > 50k emails/เดือน
- [ ] คง Resend ไว้สำหรับ transactional (OTP, notifications)
- [ ] Bounce management — auto-stop ส่งถ้า hard bounce

### Public API & Developer Experience
- [ ] API key generation (per workspace) — settings page
- [ ] API key authentication middleware
- [ ] Public REST API endpoints:
  - [ ] `GET /api/v1/public/leads` — list leads
  - [ ] `POST /api/v1/public/leads` — create lead
  - [ ] `GET /api/v1/public/campaigns` — list campaigns
  - [ ] `POST /api/v1/public/campaigns/{id}/send` — trigger send
- [ ] Rate limiting per API key (plan-based)
- [ ] API documentation (Swagger / Redoc)
- [ ] API usage logs & analytics

### Webhook System (Outgoing)
- [ ] Webhook endpoint registration (settings page)
- [ ] Webhook events: lead.created, lead.scored, email.sent, email.opened, email.bounced, campaign.completed
- [ ] Webhook delivery with retry (3 attempts, exponential backoff)
- [ ] Webhook signature verification (HMAC-SHA256)
- [ ] Webhook logs UI (delivery status, response, retry)

### Integrations
- [ ] Zapier integration (Triggers + Actions)
- [ ] HubSpot CRM sync (push leads + contacts)
- [ ] Salesforce CRM sync
- [ ] Google Sheets export (auto-sync)
- [ ] Slack notifications (campaign complete, daily digest)
- [ ] LINE Notify (สำหรับตลาดไทย)

### Performance & Infrastructure
- [ ] Places API cache optimization (เต็มรูปแบบ)
...
- [ ] Custom domain สำหรับ client portal (CNAME setup)
- [ ] Custom branding (logo, colors) per workspace
- [ ] Custom email footer per workspace
- [ ] Remove "Powered by" branding (Agency plan)
